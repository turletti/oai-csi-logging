...
if ! grep -q "csi_rb_logging_enabled && srs_estimated_channel_freq" "$FILE"; then
  awk '/^int nr_srs_channel_interpolation/ {in_func=1} in_func && /[[:space:]]*return 0;/ && !done {print "  /* Log CSI for uplink SRS channel estimation at gNB */"; print "  if (csi_rb_logging_enabled && srs_estimated_channel_freq) {"; print "    uint16_t start_rb = 0;"; print "    uint16_t nr_of_rbs = ofdm_symbol_size / 12;"; print "    nr_srs_csi_logging_invoke(frame_rx, slot_rx, start_rb, nr_of_rbs,"; print "                              srs_estimated_channel_freq, ofdm_symbol_size);"; print "  }"; print ""; done=1} {print}' "$FILE" > "$FILE.tmp" && mv "$FILE.tmp" "$FILE" && echo "✅ Added CSI logging call"
fi
